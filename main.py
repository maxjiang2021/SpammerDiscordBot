...
"""
            ...
            if user_text.lower().split(" ") == ["game","work"]:
                await message.channel.send(
"""\
Syntax: `spammer: timer <tick> <length>`
Timer progressbar with a length of `<length>` increasing by `1` every `<tick>` seconds
"""
                )
    ...
    if is_command(message.content,"spammer: game work"):
        if str(message.author.id) in replit.db.keys():
            ...
            for i in questions_fetched:
                questions.append(Question(client=client,answer=i.answer,question=i.question,channel=message.channel,user=message.author,options=i.options))
